...
class DowkerRipsComplexGudhi(TransformerMixin, BaseEstimator):
    """Class implementing the Dowker-Rips persistent homology associated to a
    point cloud whose elements are separated into two classes. This uses GUDHI
    for computation, and is present in this repository only for comparison of
    runtimes.
    ...
    Attributes:
        ...
        simplices_ (dict of int: dict of str: numpy.ndarray): Dictionary whose
            keys are the integers 0 and 1, and whose values are dictionaries
            containing the arguments to `gudhi.SimplexTree.insert_batch`. That
            is, each of these dictionaries has `"vertex_array"` and
            `"filtrations"` as keys, and NumPy-arrays of shape
            (dim + 1, n_simplices) and (n_simplices,), respectively, as its
            values. These simplices specify the 1-skeleton of the Dowker-Rips
            complex.
        complex_ (gudhi.SimplexTree): The Dowker-Rips simplicial complex
            constructed from the vertices and witnesses.
        persistence_ (list[numpy.ndarray]): The persistent homology computed
            ...
            a list of NumPy-arrays of dtype float64 of shape
    ...
    def fit(
        ...
        """Method that fits a `DowkerRipsComplexGudhi`-instance to a pair of
        point clouds consisting of vertices and witnesses by constructing the
        associated Dowker-Rips complex, as an instance of `gudhi.SimplexTree`.
        ...
        Returns:
            self (DowkerRipsComplexGudhi): The fitted instance of
                `DowkerRipsComplexGudhi`.
        """
    ...
    def transform(
        self,
        X: list[npt.NDArray],  # noqa: ARG002
    ) -> list[npt.NDArray[np.float64]]:
        """Method that transforms a `DowkerRipsComplexGudhi`-instance fitted to
        a pair of point clouds consisting of vertices and witnesses by
        computing the persistent homology of the associated Dowker-Rips
        complex.
        ...
        Returns:
            list[numpy.ndarray]: The persistent homology computed from the
                ...
                list of NumPy-arrays of dtype float64 and of shape
    ...
    def _get_complex(self):
        ...
        for simplices in self.simplices_.values():
            simplex_tree_.insert_batch(**simplices)
        simplex_tree_.expansion(self.max_dimension + 1)
